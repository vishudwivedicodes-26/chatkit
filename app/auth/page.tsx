...
export default function AuthPage() {
  ...
  return (
    <div className="relative min-h-screen flex flex-col items-center justify-center bg-[#f3f4f6] font-sans selection:bg-emerald-100 p-6 antialiased">
      <div className="relative z-10 w-full max-w-[390px] flex flex-col gap-10 py-10 animate-in fade-in duration-700">
        
        {/* 'Fitted' Card Architecture */}
        <div className="relative w-full">
          <div className="absolute inset-0 bg-white rounded-[40px] border border-zinc-200 shadow-[0_32px_64px_-16px_rgba(0,0,0,0.1)]" />
          
          <div className="relative p-10 pb-12 flex flex-col items-center gap-10">
            {/* Precision Branding Section */}
            <div className="flex flex-col items-center text-center gap-6">
              <div className="w-16 h-16 bg-[#ecfdf5] rounded-[24px] flex items-center justify-center border border-[#d1fae5] shadow-sm">
              ...
              <div className="space-y-1">
                ...
                <p className="text-[#9ca3af] text-[10px] font-bold tracking-[0.24em] uppercase">Advanced Encrypted Messenger</p>
              </div>
            </div>
            ...
            <div className="flex bg-[#f9fafb] p-1 rounded-2xl border border-zinc-200 w-full shadow-inner">
              <button 
                ...
                className={`flex-1 py-3.5 rounded-xl text-[12px] font-extrabold tracking-tight transition-all duration-300 ${
                  mode === "login" 
                    ? "bg-white text-zinc-900 shadow-md border border-zinc-100" 
            ...
            <div className="w-full flex flex-col gap-10">
              {/* Balanced Introductory Text */}
              <div className="space-y-3 px-1">
                <h2 className="text-2xl font-bold text-zinc-900 tracking-tight leading-tight" style={{ textWrap: 'balance' } as any}>
                  {mode === 'login' ? 'Welcome back' : 'Join the network'}
                </h2>
                <p className="text-zinc-500 text-[14px] font-medium leading-normal" style={{ textWrap: 'balance' } as any}>
                  {mode === 'login' ? 'Sign in to your ChatKit account.' : 'Create your secure profile on the network.'}
                </p>
              </div>

              {error && (
                <div className="bg-red-50 text-red-500 text-[13px] font-bold py-4 px-5 rounded-2xl border border-red-100 flex items-center gap-4 mx-1">
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
                  {error}
                </div>
              )}

              {/* Mathematically Aligned Fields */}
              ...
              <div className="flex flex-col gap-8">
                <button 
                  ...
                  onClick={handleAuth}
                  className={`group relative w-full h-[64px] rounded-2xl text-[16px] font-extrabold tracking-tight transition-all duration-300 active:scale-[0.98] ${
                    valid 
                      ? loading ? "bg-emerald-700" : "bg-emerald-500 text-white shadow-xl shadow-emerald-500/10 hover:bg-emerald-400" 
                ...
                >
                  {loading ? (
                    <div className="flex items-center justify-center gap-3">
                      ...
                      <span className="font-extrabold tracking-widest text-[11px] uppercase opacity-70">Authenticating</span>
                    </div>
                  ) : (
                ...
                <div className="flex flex-col gap-6">
                  <div className="relative">
                    ...
                    <div className="relative flex justify-center text-[11px] uppercase"><span className="bg-white px-3 text-zinc-300 font-bold tracking-[0.2em]">or</span></div>
                  </div>
                  ...
                  <p className="text-center text-zinc-500 text-[13px] font-medium tracking-tight">
                    ...
                    <button 
                      ...
                      className="text-emerald-500 font-extrabold hover:text-emerald-600 transition-all ml-1"
                    >
              ...
              {/* Secure Privacy Banner - 100% Fit */}
              <div className="flex items-start gap-4 p-5 bg-[#ecfdf5] border border-[#d1fae5] rounded-[32px] mt-4 mx-1">
                ...
                <p className="text-[12px] text-[#065f46] text-left font-semibold leading-relaxed tracking-tight flex-1">
...
function Field({ label, value, onChange, placeholder, password }: { label: string; value: string; onChange: (v: string) => void; placeholder: string; password?: boolean }) {
  ...
  return (
    <div className="flex flex-col gap-2">
      <label className={`block text-[13px] font-extrabold pl-5 transition-all duration-300 ${focused ? "text-zinc-900" : "text-zinc-600"}`}>
        {label}
      </label>
      <div className="relative group">
        ...
        {password && (
          <button 
            ...
            className="absolute right-5 top-1/2 -translate-y-1/2 p-2 text-zinc-500 hover:text-white active:scale-95 transition-all"
          >
            {show ? (
